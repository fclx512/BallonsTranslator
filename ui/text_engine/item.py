...
class TextBlkItem(QGraphicsTextItem):
    ...
    def keyPressEvent(self, e: QKeyEvent) -> None:
        ...
        # 标准键位匹配：兼容非英文键盘布局与 Ctrl+Insert / Shift+Insert 别名
        # （上游 a86ebb1）。
    ...
    def is_editting(self) -> bool:
        # Fork compatibility: the fork-adapted geometry controller queries
        # ``item.is_editting()`` (fork name); the engine item only ships
        # ``isEditing``.
