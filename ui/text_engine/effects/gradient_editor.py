...
class GradientValueEditor(NoArrowsDoubleSpinBox):
    """Spin box that distinguishes live text preview from one commit.

    Inherits the Blender-style drag interaction from ``DragAdjustMixin``
    (via ``NoArrowsDoubleSpinBox``); the effect card wraps drag_started /
    drag_finished for its preview-commit cycle.
    """
...
class InlineLinearGradientEditor(QWidget):
    ...
    Fork balanced trim: no per-stop opacity/position spin boxes — stops are
    added by clicking the strip, moved by dragging, recolored by the swatch;
    only the angle+scale row carries numeric editors.
    """
    ...
    def __init__(
        ...
        self.add_stop_button.setIcon(
            QIcon(themed_icon_path('add.svg'))
        )
    ...
    def _drag_value_control(
        ...
        label = QLabel(title, control)
        label.setObjectName('TextEffectParamLabel')
        label.setAlignment(
            Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter
        )
        label.setAttribute(Qt.WidgetAttribute.WA_TranslucentBackground, True)
        # 拖拽由输入框本体承担（NoArrowsDoubleSpinBox 继承的 DragAdjustMixin）：
        # valueChanged 接 live preview；drag_started 先落上一笔再开预览事务，
        # drag_finished 提交。标签仅作说明文字，不再扛拖拽。
        editor.drag_started.connect(self._on_editor_drag_started)
        editor.drag_finished.connect(self._commit_current)
    ...
    def _on_editor_drag_started(self) -> None:
        editor = self.sender()
        editor.resolve_text_edit()
        self._commit_current()

    def _editors(self) -> Tuple[GradientValueEditor, ...]:
        return (self.angle_editor, self.scale_editor)
