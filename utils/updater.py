"""GitHub-release updater for BallonsTranslator-lite.

Ported from upstream ``dmMaze/BallonsTranslator`` (utils/updater.py) and
adapted to this fork's repository layout:

- checks ``fclx512/BallonsTranslator-lite`` latest release instead of a commit
- compares versions with semver (falls back to a conservative tuple compare)
- applies updates by downloading the release source zip, backing up the
  current source, stashing local git changes, and atomically replacing the
  tracked source paths (user data under gitignore is never touched).
"""
...
# Source paths managed by the updater. ``data/`` and gitignored user data
# (config.json, models, ballontrans_pylibs_win, .btrans_cache, ...) are
# intentionally excluded so an update never deletes user files.
SOURCE_UPDATE_DIRS = (
    "modules",
    "ui",
    "utils",
...
SOURCE_UPDATE_FILES = (
    ...
    "install_cuda.bat",
    ".gitignore",
    # config/ is split into per-file whitelist entries so the user's
...
def is_remote_newer(local_version: str, remote_version: str) -> bool:
    ...
    >>> is_remote_newer('0.4.0', 'v0.5.0')
    True
    >>> is_remote_newer('0.5.0', 'v0.5.0')
    False
    """
...
def release_info_from_api_payload(payload: dict) -> ReleaseInfo:
    ...
    >>> info = release_info_from_api_payload({'tag_name': 'v0.5.0', 'html_url': 'u', 'zipball_url': 'z', 'body': 'notes'})
...
class BallonsTranslatorUpdater:
    ...
    def apply_update(self, release_info: ReleaseInfo, current_version: str = None) -> UpdateResult:
        current_version = current_version or get_version(str(self.program_path))
        self.backup_source()
    ...
    def query_latest_release(self) -> ReleaseInfo:
        ...
        request = Request(
            ...
            headers={
                ...
                "User-Agent": "BallonsTranslator-lite-Updater",
            },
        )
        try:
    ...
    def download_source_zip(self, release_info: ReleaseInfo) -> Path:
        self.cache_dir.mkdir(parents=True, exist_ok=True)
        zip_path = self.cache_dir / f"BallonsTranslator-lite_{release_info.version}_source.zip"
        self._notify("download_start", 20, zip_path.name)
        self._download_zip(release_info.zip_url, zip_path)
        return zip_path

    def _download_zip(self, url: str, zip_path: Path) -> None:
        """Download the release source zip with progress notifications.

        Uses urllib directly (upstream's ``download_url_to_file`` in this fork
        has no progress callback and prints a tqdm bar to stderr instead).
        """
        request = Request(url, headers={"User-Agent": "BallonsTranslator-lite-Updater"})
                ...
                with temp_path.open("wb") as f:
                    while True:
                        ...
                        self._on_download_progress(
                            {"event": "file_progress", "total": total, "downloaded": downloaded}
                        )
    ...
    def backup_source(self) -> Path:
        """Snapshot every updater-managed installed path."""
    ...
    def prepare_git_worktree(self, latest_version: str) -> str:
        ...
        if status:
            ...
            self._git(["add", "-A"])
            stash_message = f"BallonsTranslator-lite updater local changes before {latest_version}"
    ...
    def _result_for_release(self, release_info: ReleaseInfo) -> UpdateResult:
        current_version = get_version(str(self.program_path))
        ...
        LOGGER.info(f"BallonsTranslator-lite update available: {current_version} -> {release_info.version}")
        return UpdateResult(
            status="available",
            current_version=current_version,
            latest_version=release_info.version,
    ...
    def _on_download_progress(self, payload: dict) -> None:
        ...
        if event == "file_progress":
            ...
            self._notify("download_progress", progress, os.path.basename(str(payload.get("path", ""))))
        elif event == "file_done":
            self._notify("download_done", 60, os.path.basename(str(payload.get("path", ""))))
    ...
    def _find_release_root(self, extract_root: Path) -> Path:
        for candidate in extract_root.iterdir():
            if candidate.is_dir() and (candidate / "launch.py").is_file() and (candidate / "modules").is_dir():
                return candidate
        raise RuntimeError("Downloaded source archive does not contain the BallonsTranslator-lite source.")
